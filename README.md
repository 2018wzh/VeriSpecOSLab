...
- `vos/`：Bun / TypeScript workspace，实现统一命令入口、Agent Gateway、Portal 后端与前端
...
└── vos/
    ...
    │   ├── vos-agent-core   # agent context/plan/apply-patch/log 的确定性编排
    │   └── vos-cli          # bin: vos
    └── apps/
        ├── vos-agent        # LLM runner、Agent Gateway、Portal API 后端
        └── vos-web          # Portal 前端
```

当前代码已经包含 `apps/vos-agent` 与 `apps/vos-web`；`packages/*`
是后续把 CLI / runtime / evidence / policy 从设计落地为共享 TypeScript
模块时的目标边界。
...
- `vos/apps/vos-web` 已实现 Portal 前端原型
- `vos/packages/*` 所描述的 CLI / runtime / evidence / policy package 仍是目标改造方向
