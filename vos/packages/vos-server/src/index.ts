...
const routes: RouteDef[] = [
  ...
  { method: "POST", path: "/api/v1/agent/ask-runs", kind: "run", schema: z.object({ ...commonRunFields, question: z.string().min(1), scope: z.string().optional() }).strict(), makeCommand: (input) => ({ kind: "agent_ask", question: input.question as string, scope: stringValue(input.scope) }) },
