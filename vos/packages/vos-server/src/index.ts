...
const routes: RouteDef[] = [
  ...
  { method: "POST", path: "/api/v1/verify/runs", kind: "run", schema: z.object({ ...commonRunFields, scope: z.enum(["public", "full"]).optional(), target: z.string().optional(), dry_run: z.boolean().optional(), hidden: z.boolean().optional() }).strict(), makeCommand: (input) => ({ kind: "verify", scope: input.scope === "full" ? "full" : "public", target: stringValue(input.target), dryRun: bool(input.dry_run), hidden: bool(input.hidden) }) },
