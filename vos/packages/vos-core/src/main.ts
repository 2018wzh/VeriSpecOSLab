...
export async function executeAgentImplement(
  ...
  // Force an early structured checkpoint while retaining most of the hard
  // iteration budget for evidence-driven repair after a rejected payload.
  // A large maxIterations value must not postpone the first submission until
  // the provider quota has already been consumed by redundant validation.
  const implementationCompletionReserveIterations = 840;
  const implementationSubmissionCheckpoint = implementationMaxIterations
    - implementationCompletionReserveIterations
    + 1;
  const implementationLastRepairIteration = implementationMaxIterations - 1;
  ...
  try {
    ...
    while (true) {
      ...
      const agentResult = await runAgentWithPrompt({
        ...
        completionReserveIterations: implementationCompletionReserveIterations,
