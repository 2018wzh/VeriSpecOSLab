...
export async function executeAgentImplement(
  ...
  const implementationMaxIterations = 100;
  const implementationCompletionReserveIterations = 10;
  ...
  try {
    ...
    taskPrompt += `\nThe hard maxIterations value is ${implementationMaxIterations}. Iteration 91 is the structured submission checkpoint. If it is rejected, iterations 92-99 restore the full tool set for repair and iteration 100 requires the corrected submission.`;
    ...
    while (true) {
      ...
      if (validation.status === "passed" || validation.status === "owns_violation" || usedIterations >= implementationMaxIterations || !threadId) break;
