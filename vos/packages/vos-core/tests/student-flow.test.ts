...
describe("student v2 workflow", () => {
    ...
    await withGitIdentity(async () => {
      ...
      const result = await executeCliInvocation(["bun", "vos", "--project-root", root, "--json", "agent", "implement", "memory"], {
        ...
        agentRunner: async (options) => {
          ...
          expect(options.task).toContain("there is no intermediate iteration checkpoint");
          expect(options.task).toContain("submit as soon as the work is complete");
  ...
  test("returns unsupported manifest projection fields to the same Agent thread", async () => {
    const root = makeRoot();
    await withGitIdentity(async () => {
      await prepareModuleProject(root);
      let turn = 0;
      const result = await executeCliInvocation(["bun", "vos", "--project-root", root, "--json", "agent", "implement", "memory"], {
        ...
        agentRunner: async (options) => {
          ...
          const proposal = implementationResult();
          if (turn === 1) {
            (proposal.test_targets[0] as Record<string, unknown>).state = "verified";
          } else {
            expect(options.threadId).toBe("projection-field-repair-thread");
            expect(options.task).toContain("rejected the structured implementation result");
            expect(options.task).toContain("unsupported field(s): state");
          }
          return {
            content: "submitted",
            threadId: "projection-field-repair-thread",
            events: [
              { type: "model.usage", thread_id: "projection-field-repair-thread", iteration: turn === 1 ? 45 : 2 },
              ...acceptedSubmitEvents("student_implementation_result.v1", proposal).map((event) => ({ ...event, thread_id: "projection-field-repair-thread", iteration: turn === 1 ? 45 : 2 })),
            ],
          };
        },
      });
