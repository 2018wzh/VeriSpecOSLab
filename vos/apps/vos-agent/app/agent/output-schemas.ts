...
const schemas: Record<string, OutputSchemaDefinition> = {
  ...
  "qemu_preflight_result.v1": {
    id: "qemu_preflight_result.v1",
    description: "Evidence-gated QEMU physical-board preflight result.",
    schema: objectSchema({
      status: { type: "string", enum: ["sufficient", "insufficient"] },
      missing: { type: "array", items: stringObject },
      reasons: stringArray,
      boot_path: { type: "object", additionalProperties: true },
      reuse_matrix: { type: "array", items: stringObject },
      findings: { type: "array", items: stringObject },
      phases: { type: "array", items: stringObject },
      dependencies: { type: "array", items: stringObject },
      owns: stringArray,
      notes: stringArray,
      summary: { type: "string" },
    }, ["status", "missing", "reasons", "boot_path", "reuse_matrix", "findings", "phases", "dependencies", "owns", "notes", "summary"]),
  },
  "qemu_execution_result.v1": {
    id: "qemu_execution_result.v1",
    description: "QEMU port implementation and Agent-owned shell acceptance result.",
    schema: objectSchema({
      status: { type: "string", enum: ["passed", "blocked"] },
      changed_paths: stringArray,
      validations: stringArray,
      phase_commits: stringArray,
      dependency_pins: { type: "array", items: stringObject },
      summary: { type: "string" },
      diagnostics: stringArray,
      blocker: { type: "string" },
      resume_steps: stringArray,
    }, ["status", "changed_paths", "validations", "phase_commits", "dependency_pins", "summary", "diagnostics", "resume_steps"]),
  },
