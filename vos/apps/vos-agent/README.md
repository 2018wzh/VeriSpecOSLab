...
VOS Agent is an Amp-style terminal coding agent and TypeScript backend
for VeriSpecOSLab. It can run interactively (`vos-agent`) or headlessly
(`vos-agent -p "..."`),
persists local threads, injects `AGENTS.md` guidance, tracks todos, and
routes each model request to Anthropic's Messages API or an
OpenAI-compatible endpoint. In server mode it also serves the VOS portal
REST API consumed by `apps/vos-web`.
...
Stars currently ships these tool-call surfaces:
