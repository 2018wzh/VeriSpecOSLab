...
import { startPortalServer } from "../server/http.ts";
...
integration("worker evidence control HTTP path verifies MinIO metadata before committing evidence", async () => {
  ...
  try {
    ...
    await sql.begin(async (tx) => {
      ...
      await tx`insert into pipeline_runs(id,project_id,commit_sha,stage_key,scope,status,policy_snapshot_ref,requested_by,reason,lease_owner,leased_until) values(${run},${project},${"b".repeat(40)},'boot','public','running','policy-worker-http',${user},'worker HTTP integration',${workerId},now()+interval '2 minutes')`;
    });
    ...
    await client.reportEvidence(run, {
      ...
      objects: [{ id: `${prefix}-object`, key, uri: stored.uri, sha256: stored.sha256, size_bytes: stored.size_bytes, content_type: "text/plain", visibility: "student", label: "HTTP evidence", lineage: { source: "worker-http" } }],
