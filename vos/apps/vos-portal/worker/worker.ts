...
export function portalRunStatus(status: string):
  | "passed"
  | "failed"
  | "cancelled"
  | "timed_out" {
...
async function waitForRun(
  ...
  while (Date.now() < deadline && !signal.aborted) {
    if(Date.now()-lastHeartbeatAt>=20_000){const heartbeat=await control.heartbeat(portalRunId);lastHeartbeatAt=Date.now();if(heartbeat.run_status==="cancelled"){
      await fetch(
...
export async function runWorker(signal: AbortSignal): Promise<void> {
  ...
  const modelControl=await ModelControlService.create(sql,required("VOS_PORTAL_MASTER_KEY"));
  const objectStore = S3ObjectStore.fromEnv();
  const credentials=await ModelCredentialService.create(sql,required("VOS_PORTAL_MASTER_KEY"));
  let lastWorkerHeartbeatAt=0;
  while (!signal.aborted) {
    if(Date.now()-lastWorkerHeartbeatAt>=20_000){await control.heartbeat();lastWorkerHeartbeatAt=Date.now();}
    if(await recoverExpiredPipelineLeaseOne(sql,workerId,async runId=>await runtime.cleanupRun(runId)))continue;
    ...
    await control.heartbeat(String(run.id));lastWorkerHeartbeatAt=Date.now();
    const terminalContext={remoteRunId:"not-started",evidence:0,objects:0,imageId:"not-started",containerId:"not-started"};
    try {
      const modelCredential=await credentials.leaseForRun(String(run.id),workerId,`worker-${crypto.randomUUID()}`);
      ...
      terminalContext.imageId=session.imageId;terminalContext.containerId=session.containerId;
      try {
      ...
      if (!response.ok)
        throw new Error(`vos serve rejected run: HTTP ${response.status}`);
      ...
      let collected;
      try{collected = finalStatus === "cancelled" || finalStatus === "timed_out"
          ? undefined : await collectRunnerEvidence({
              store: objectStore,
              workerId,
              endpoint: base,
            ...
            });}catch(error){const files=await session.diagnostics().catch(()=>[]);throw new Error(`${error instanceof Error?error.message:String(error)}; runner_error=${completed.error??"none"}; runner_files=${JSON.stringify(files)}`);}
      if (collected) {
      ...
      if(collected){await control.reportEvidence(String(run.id),collected.report);terminalContext.evidence=collected.report.evidence.length;terminalContext.objects=collected.report.objects.length;}
      await control.complete(String(run.id),{version:"worker-run-complete.v1",worker_id:workerId,remote_run_id:remote.run_id,status:finalStatus,failure_class:finalStatus==="failed"?"verification_failure":undefined,runner_error:completed.error,manifest_status:collected?.manifest.status,manifest_message:collected?.manifest.message,evidence_records:collected?.report.evidence.length??0,objects:collected?.report.objects.length??0,runner_image_id:session.imageId,runner_container_id:session.containerId});
      } finally {
      ...
      await control.complete(String(run.id),{version:"worker-run-complete.v1",worker_id:workerId,remote_run_id:terminalContext.remoteRunId,status:"failed",failure_class:"infra_failure",runner_error:detail,evidence_records:terminalContext.evidence,objects:terminalContext.objects,runner_image_id:terminalContext.imageId,runner_container_id:terminalContext.containerId}).catch(controlError=>console.error(JSON.stringify({level:"error",event:"pipeline_worker_failure_report_rejected",worker_id:workerId,run_id:String(run.id),error:controlError instanceof Error?controlError.message:String(controlError)})));
    }
  }
}

export async function dispatchAuditOutboxOne(sql:Sql,workerId:string):Promise<boolean>{
  const rows=await sql.begin(async tx=>await tx`update outbox_events set lease_owner=${workerId},leased_until=now()+interval '2 minutes',attempts=attempts+1 where id=(select id from outbox_events where topic='audit.recorded' and published_at is null and next_attempt_at<=now() and (leased_until is null or leased_until<now()) order by created_at,id limit 1 for update skip locked) returning id,payload`);
  const event=rows[0];
  if(!event)return false;
  console.info(JSON.stringify({level:"info",event:"audit_outbox_published",worker_id:workerId,...(event.payload as Record<string,unknown>)}));
  const published=await sql`update outbox_events set published_at=now(),lease_owner=null,leased_until=null,last_error=null where id=${String(event.id)} and lease_owner=${workerId} and published_at is null returning id`;
  if(!published[0])throw new Error("audit outbox lease ownership was lost before publish acknowledgement");
  return true;
}

export async function recoverExpiredPipelineLeaseOne(sql:Sql,workerId:string,cleanup:(runId:string)=>Promise<number>):Promise<boolean>{
  const recoveryOwner=`${workerId}.recovery`;
  const claimed=await sql.begin(async tx=>await tx`with candidate as (select id,lease_owner from pipeline_runs where status in ('leased','running') and leased_until<now() order by leased_until,id limit 1 for update skip locked), updated as (update pipeline_runs run set lease_owner=${recoveryOwner},leased_until=now()+interval '2 minutes' from candidate where run.id=candidate.id returning run.id) select updated.id,candidate.lease_owner previous_owner from updated join candidate on candidate.id=updated.id`);
  if(!claimed[0])return false;
  const runId=String(claimed[0].id);const previousOwner=String(claimed[0].previous_owner??"unknown");let removed:number;
  try{removed=await cleanup(runId);}catch(error){const detail=error instanceof Error?error.message:String(error);await sql.begin(async tx=>{await tx`update pipeline_runs set leased_until=now() where id=${runId} and lease_owner=${recoveryOwner}`;await tx`insert into audit_events(id,actor_id,action,resource_type,resource_id,reason,trace_id,payload) values(${`audit-${crypto.randomUUID()}`},null,'runner.recovery.failure','pipeline',${runId},'expired runner cleanup failed',${`worker-${crypto.randomUUID()}`},${tx.json({worker_id:workerId,previous_owner:previousOwner,error:detail})})`;});throw error;}
  await sql.begin(async tx=>{const failed=await tx`update pipeline_runs set status='failed',failure_class='infra_failure',public_message='评测 worker 租约过期，已安全终止，请由课程团队批准补跑',finished_at=now(),lease_owner=null,leased_until=null where id=${runId} and lease_owner=${recoveryOwner} and status in ('leased','running') returning id`;if(!failed[0])throw new Error("expired pipeline recovery lease was lost");await tx`update model_credential_leases set revoked_at=coalesce(revoked_at,now()) where run_id=${runId}`;const seq=await tx`select coalesce(max(sequence),-1)+1 sequence from pipeline_events where run_id=${runId}`;await tx`insert into pipeline_events(run_id,sequence,event_type,visibility,payload) values(${runId},${Number(seq[0].sequence)},'finished','student',${tx.json({status:"failed",failure_class:"infra_failure"})})`;await tx`insert into audit_events(id,actor_id,action,resource_type,resource_id,reason,trace_id,payload) values(${`audit-${crypto.randomUUID()}`},null,'runner.recovered_expired_lease','pipeline',${runId},'expired runner lease was recovered and failed closed',${`worker-${crypto.randomUUID()}`},${tx.json({worker_id:workerId,previous_owner:previousOwner,containers_removed:removed})})`;});
  console.warn(JSON.stringify({level:"warn",event:"pipeline_expired_lease_recovered",worker_id:workerId,run_id:runId,previous_owner:previousOwner,containers_removed:removed}));return true;
}
