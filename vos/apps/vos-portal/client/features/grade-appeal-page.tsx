...
export function GradeAppealPage({ appealsOnly = false }: { appealsOnly?: boolean }) {
  ...
  const appeals = useQuery({ queryKey: ["appeals", projectId], queryFn: () => repository.appeals(projectId!), enabled: Boolean(projectId) });
  const [reason, setReason] = useState("");
  const [statement, setStatement] = useState("");
  const [delta, setDelta] = useState("0");
  const [message, setMessage] = useState("");

  if (dashboard.isLoading) return <div className="page-loading">{t("正在加载评分记录…")}</div>;
  if (dashboard.error || !dashboard.data) return <div className="error-state">{t("评分上下文不可用")}</div>;
    ...
    <div className="grade-grid">
      ...
      {!appealsOnly && teacher ? <section className="surface grade-actions">
        ...
        <div className="action-row">
          <button className="button outline" disabled={reason.trim().length < 10} onClick={() => void mutate(() => repository.calculateScore({ project_id: projectId!, reason }))}>{t("计算基线")}</button>
          <button className="button outline" disabled={reason.trim().length < 10 || !evidenceRef || score.state !== "draft"} onClick={() => void mutate(() => repository.updateScore({ project_id: projectId!, member_id: data.project.member_ids[0], delta: Number(delta), reason, evidence_refs: [evidenceRef!] }))}>{t("记录调整")}</button>
          <button className="button outline" disabled={reason.trim().length < 10 || score.state !== "draft" || score.snapshot_version === 0} onClick={() => void mutate(() => repository.transitionScore({ score_snapshot_id: score.id, target_state: "frozen", reason }))}>{t("冻结成绩")}</button>
          <button className="button primary" disabled={reason.trim().length < 10 || score.state !== "frozen"} onClick={() => void mutate(() => repository.transitionScore({ score_snapshot_id: score.id, target_state: "published", reason }))}>{t("发布成绩")}</button>
          <button className="button outline" disabled={reason.trim().length < 10 || data.course.status !== "grading"} onClick={() => void mutate(() => repository.transitionCourse(data.course.id, "appeal", reason))}>{t("开放申诉")}</button>
      ...
      <section className="surface appeal-panel">
        ...
        {student && data.course.status === "appeal" && score.state === "published" ? <div className="appeal-submit"><label className="reason">{t("申诉说明")}<textarea value={statement} onChange={(event) => setStatement(event.target.value)} placeholder={t("至少 20 个字符，并关联最近一次运行证据")} /></label><button className="button primary" disabled={statement.trim().length < 20 || !evidenceRef} onClick={() => void mutate(() => repository.submitAppeal({ project_id: projectId!, statement, evidence_refs: [evidenceRef!] }))}>{t("提交申诉")}</button></div> : null}
        <div className="structured-list">{appeals.data?.length ? appeals.data.map((appeal) => <div className="appeal-record" key={appeal.id}><span><b>{t(appeal.status)}</b><small>{appeal.statement}</small>{appeal.decision ? <small>{t("裁决")}：{appeal.decision}</small> : null}</span>{!student ? <span className="appeal-actions">{appeal.status === "submitted" ? <button className="button outline" disabled={reason.trim().length < 10} onClick={() => void changeAppeal({ appeal_id: appeal.id, target_status: "fact_check", reason })}>{t("完成核查")}</button> : null}{teacher && appeal.status === "fact_check" ? <button className="button primary" disabled={reason.trim().length < 10} onClick={() => void changeAppeal({ appeal_id: appeal.id, target_status: "decision", reason, decision: reason, score_delta: Number(delta) })}>{t("作出裁决")}</button> : null}{teacher && appeal.status === "decision" ? <button className="button outline" disabled={reason.trim().length < 10} onClick={() => void changeAppeal({ appeal_id: appeal.id, target_status: "closed", reason })}>{t("关闭")}</button> : null}</span> : null}</div>) : <div><b>{t("暂无申诉")}</b><span>{t("申诉窗口开放后可提交带证据引用的材料。")}</span></div>}</div>
