...
export function StudentDashboard() {
  ...
  return <div className="page student-home">
    ...
    <section className="primary-action surface"><div><small>{t("当前任务")} · Lab {current.sequence + 1}</small><h2>{t("继续完成 {{stage}}", { stage: current.name })}</h2><p>{t("先查看阶段门槛，再提交实现与公开证据；Portal 只负责控制面与审计。")}</p><div className="action-meta"><span>{current.required_artifacts.length} {t("项产物")}</span><span>{current.required_evidence.length} {t("项证据")}</span>{current.required_review_artifacts.length > 0 ? <span>{current.required_review_artifacts.length} {t("项复核材料")}</span> : null}{current.manual_review_required ? <span>{t("需要人工复核")}</span> : null}</div></div><Button appearance="primary" onClick={() => navigate(action.href)}>{t(action.label)} <ArrowRight24Regular /></Button></section>
