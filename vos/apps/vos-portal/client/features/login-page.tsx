...
import type { OidcProviderSummaryV1 } from "vos-core/portal-contracts";
...
export function LoginPage({ demo, onLogin }: { demo:boolean; onLogin: () => void }) {
  ...
  useEffect(()=>{let active=true;void repository.oidcProviders().then((value)=>{if(active)setProviders(value);}).catch((reason)=>{if(active)setError(reason instanceof Error?reason.message:String(reason));});return()=>{active=false;};},[repository]);
