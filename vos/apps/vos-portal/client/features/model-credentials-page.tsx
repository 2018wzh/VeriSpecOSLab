...
export function ModelCredentialsPage({ demo }: { demo: boolean }) {
  ...
  const credentials = useQuery({ queryKey: portalQueryKey(scope, "model-credentials"), queryFn: () => repository.modelCredentials(), enabled: !demo });
  ...
  if (!demo && credentials.isLoading) return <PageLoading label={t("正在加载模型凭据…")} />;
  if (!demo && credentials.isError) return <PageError message={credentials.error instanceof Error ? credentials.error.message : String(credentials.error)} retryLabel={t("重试")} onRetry={() => void credentials.refetch()} />;
  ...
  async function save() {
    ...
    try {
      ...
      setSecret("");
      setReason("");
      await queryClient.invalidateQueries({ queryKey: portalQueryKey(scope, "model-credentials") });
  ...
  return <div className="page"><div className="page-heading"><div><h1>{t("模型凭据")}</h1><p>{t("自带模型凭据由服务端包封加密，仅显示末四位；撤销操作保留审计记录。")}</p></div></div>
    {demo ? <section className="surface"><div className="empty-notice"><b>{t("Demo 不保存模型密钥")}</b><span>{t("静态演示构建不包含数据库、主密钥或模型 provider 代码路径。")}</span></div></section> : <div className="workspace-layout">
      <section className="surface"><header><h2>{t("已保存凭据")}</h2><small>{credentials.data?.length ?? 0}</small></header><div className="structured-list">
        {credentials.data?.length ? credentials.data.map((credential) => <div key={credential.id}><span><b>{credential.label}</b><small>{credential.provider} · •••• {credential.last_four}</small></span><span>{credential.revoked_at ? t("已撤销") : <Button appearance="secondary" disabled={reason.trim().length < 10} onClick={() => void revoke(credential.id)}>{t("撤销")}</Button>}</span></div>) : <div><b>{t("暂无模型凭据")}</b><span>{t("课程策略允许 BYOK 后可在此添加。")}</span></div>}
      </div></section>
      <section className="surface credential-form"><header><h2>{t("添加凭据")}</h2></header>
        ...
        <Button appearance="primary" disabled={!valid} onClick={() => void save()}>{t("加密保存")}</Button>
