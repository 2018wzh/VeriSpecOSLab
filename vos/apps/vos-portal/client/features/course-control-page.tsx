...
export function CourseControlPage() {
  ...
  const versions = useQuery({
    queryKey: portalQueryKey(scope, "course-manifest-versions"),
    queryFn: () => repository.courseManifestVersions(courseId!),
    enabled: Boolean(courseId),
  });
  const groups = useQuery({ queryKey: portalQueryKey(scope, "course-groups"), queryFn: () => repository.courseGroups(courseId!), enabled: Boolean(courseId) });
  const invites = useQuery({ queryKey: portalQueryKey(scope, "enrollment-invites"), queryFn: () => repository.enrollmentInvites(courseId!), enabled: Boolean(courseId) });
  const provisionOptions = useQuery({ queryKey: portalQueryKey(scope, "project-provision-options"), queryFn: () => repository.projectProvisionOptions(), enabled: Boolean(courseId) });
    ...
    <div className="course-control-layout">
      <section className="surface course-editor">
        ...
        <div className="action-row">
          <Button appearance="secondary" disabled={busy} onClick={() => void run(async () => {
          ...
          <Button appearance="primary" disabled={busy || reason.trim().length < 10} onClick={() => void run(async () => {
      ...
      <section className="surface version-history">
        ...
        {versions.data?.length ? versions.data.map(item => <article key={item.manifest_version}>
          ...
          <div className="version-actions">
            {item.state === "draft" ? <Button appearance="primary" disabled={busy || reason.trim().length < 10} onClick={() => void run(async () => { await repository.publishCourseManifest(item.course_id, item.manifest_version, reason); return t("课程清单 v{{version}} 已发布", { version: item.manifest_version }); })}>{t("发布")}</Button> : null}
            {item.state === "superseded" ? <Button appearance="secondary" disabled={busy || reason.trim().length < 10} onClick={() => void run(async () => { const restored = await repository.rollbackCourseManifest(item.course_id, item.manifest_version, reason); return t("历史 v{{old}} 已恢复为新快照 v{{next}}", { old: item.manifest_version, next: restored.manifest_version }); })}>{t("回滚到此内容")}</Button> : null}
          </div>
      ...
      <section className="surface enrollment-import">
        ...
        <div className="action-row">
          <Button appearance="secondary" disabled={busy || !courseId || reason.trim().length < 10} onClick={() => void run(async () => { const result = await repository.importEnrollmentCsv({ course_id: courseId!, csv, dry_run: true, reason }); if (result.issues.length) throw new Error(result.issues.map(issue => t("第 {{row}} 行：{{message}}",{row:issue.row,message:issue.message})).join("；")); return t("名单校验通过：{{count}} 条记录",{count:result.accepted}); })}>{t("名单 Dry-run")}</Button>
          <Button appearance="primary" disabled={busy || !courseId || reason.trim().length < 10} onClick={() => void run(async () => { const result = await repository.importEnrollmentCsv({ course_id: courseId!, csv, dry_run: false, reason }); if (result.issues.length) throw new Error(t("名单存在错误，未应用")); return t("已应用 {{memberships}} 条成员关系，新建 {{users}} 个账号",{memberships:result.updated_memberships,users:result.created_users}); })}>{t("应用名单")}</Button>
        </div>
      </section>
      <section className="surface enrollment-invites">
        ...
        <div className="invite-form">
          ...
          <label>{t("有效天数")}<Input type="number" min={1} max={180} value={String(inviteDays)} onChange={event=>setInviteDays(Number(event.target.value))} /></label>
          <label>{t("最多使用次数")}<Input type="number" min={1} max={500} value={String(inviteMaxUses)} onChange={event=>setInviteMaxUses(Number(event.target.value))} /></label>
          <Button appearance="primary" disabled={busy||!courseId||reason.trim().length<10} onClick={()=>void run(async()=>{const issued=await repository.createEnrollmentInvite({course_id:courseId!,role:inviteRole,expires_at:new Date(Date.now()+inviteDays*24*60*60_000).toISOString(),max_uses:inviteMaxUses,reason});setIssuedInviteCode(issued.code);return t("邀请码已创建，请立即安全保存。")})}>{t("创建邀请码")}</Button>
        </div>
        {issuedInviteCode?<div className="issued-secret" role="status"><b>{t("仅显示一次")}</b><code>{issuedInviteCode}</code><Button appearance="secondary" type="button" onClick={()=>void navigator.clipboard.writeText(issuedInviteCode)}>{t("复制")}</Button></div>:null}
      ...
      <section className="surface course-groups">
        <header><div><h2>{t("小组与成员调整")}</h2><small>{t("使用乐观版本和审计理由更新；学生同一课程只能属于一个小组。")}</small></div><Button appearance="secondary" type="button" onClick={() => { setSelectedGroup(null); setGroupName(""); setGroupMembers([]); }}>{t("新建小组")}</Button></header>
        <div className="group-management">
          ...
          <form onSubmit={event => { event.preventDefault(); void run(async () => { if(!courseId)throw new Error(t("课程尚未加载"));const input={name:groupName,member_ids:groupMembers,expected_revision:selectedGroup?.revision??0,reason};const saved=selectedGroup?await repository.updateCourseGroup(courseId,selectedGroup.id,input):await repository.createCourseGroup(courseId,input);setSelectedGroup(saved);setGroupName(saved.name);setGroupMembers([...saved.member_ids]);return t(selectedGroup?"小组成员已更新":"小组已创建"); }); }}>
            ...
            <fieldset><legend>{t("课程学生")}</legend>{provisionOptions.data?.members.filter(member=>member.course_id===courseId).map(member=><div className="check" key={member.id}><Checkbox checked={groupMembers.includes(member.id)} onChange={(_, data)=>setGroupMembers(current=>data.checked === true?[...current,member.id]:current.filter(id=>id!==member.id))} label={<span>{member.display_name}<small>{member.username}</small></span>} /></div>)}</fieldset>
            <Button appearance="primary" disabled={busy||!groupName.trim()||groupMembers.length===0||reason.trim().length<10}>{t(selectedGroup?"保存小组调整":"创建小组")}</Button>
          </form>
        </div>
      </section>
    </div>
...
function exampleManifest(course: { code: string; name: string; term: string }): CourseManifestV1 {
  return {
    ...
    stages: [
      { id: "seed", key: "seed", name: "架构种子", sequence: 0, source_ref:"course/lab1-complete",spec_refs:["design"],test_sets:["lab1.public"],rubric_ids:["design"],hardware_gate:"none",human_review_required:true,required_artifacts: ["spec/architecture-seed.yaml"], required_evidence: [], required_review_artifacts: [], required_showcase_artifacts: [], manual_review_required: true },
      { id: "boot", key: "boot", name: "启动", sequence: 1, source_ref:"course/lab2-complete",spec_refs:["kernel.boot"],test_sets:["lab2.public"],rubric_ids:["correctness"],hardware_gate:"none",human_review_required:false,required_artifacts: ["serial.log"], required_evidence: [{ suite: "boot", case_name: "kernel-start", required_result: "pass" }], required_review_artifacts: [], required_showcase_artifacts: [], manual_review_required: false },
    ],
