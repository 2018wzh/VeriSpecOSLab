...
pub async fn run() {
    ...
    let command_future = async move {
        match cli.command {
            ...
            Commands::Verify {
                ...
                json,
                "vos verify patch",
                verify_patch_envelope(&project_root, &args.patch_path, progress_cb.as_deref())
                    .await,
            ),
            Commands::Verify {
    ...
    let outcome = tokio::select! {
        result = command_future => RunOutcome::Completed(result),
    ...
    match outcome {
        ...
        RunOutcome::Completed(Err(err)) => {
            ...
            print_envelope(json, &failure).expect("failed to print envelope");
            std::process::exit(1);
        }
        RunOutcome::Interrupted => {
            let failure = interrupted_envelope(command_name);
            print_envelope(json, &failure).expect("failed to print envelope");
...
fn interrupted_envelope(command: &str) -> vos_core::CommandEnvelope<FailurePayload> {
    vos_core::envelope_with_run_id(
        vos_core::new_run_id(),
        command,
        CommandStatus::Failed,
        Vec::new(),
        FailurePayload {
            ...
            message: "terminated by Ctrl+C".into(),
            diagnostics: Vec::new(),
        },
    )
}
...
fn command_name(command: &Commands) -> &'static str {
    match command {
        Commands::Init => "vos init",
        Commands::Stage {
            command: StageCommands::Show,
        } => "vos stage show",
        Commands::Spec {
            command: SpecCommands::Lint(_),
        } => "vos spec lint",
        Commands::Spec {
            command: SpecCommands::Normalize(_),
        } => "vos spec normalize",
        Commands::Spec {
        ...
        Commands::Arch {
            command: ArchCommands::Lint(_),
        } => "vos arch lint",
        Commands::Arch {
            command: ArchCommands::Compose(_),
        } => "vos arch compose",
        Commands::Arch {
            command: ArchCommands::DeriveTests(_),
        } => "vos arch derive-tests",
        Commands::Build(_) => "vos build",
        Commands::Run {
            command: RunCommands::Qemu(_),
        } => "vos run qemu",
        Commands::Test(_) => "vos test",
        Commands::Verify {
            command: VerifyCommands::Public,
        } => "vos verify public",
        Commands::Verify {
            command: VerifyCommands::Patch(_),
        } => "vos verify patch",
        Commands::Verify {
            command: VerifyCommands::Full,
        } => "vos verify full",
        ...
        Commands::Debug {
            command: DebugCommands::ExplainLog(_),
        } => "vos debug explain-log",
        Commands::Agent {
            command: AgentCommands::Serve(_),
        } => "vos agent serve",
        Commands::Agent {
            command: AgentCommands::Context(_),
        } => "vos agent context",
        Commands::Agent {
            command: AgentCommands::Plan(_),
        } => "vos agent plan",
        Commands::Agent {
            command: AgentCommands::Generate(_),
        } => "vos agent generate",
        Commands::Agent {
            command: AgentCommands::ApplyPatch(_),
        } => "vos agent apply-patch",
        Commands::Agent {
            command: AgentCommands::Log,
        } => "vos agent log",
...
mod tests {
    ...
    fn command_names_cover_nested_variants() {
        ...
        assert_eq!(
            command_name(&Commands::Agent {
                command: AgentCommands::Generate(AgentGenerateArgs {
                    target: None,
                    from_patch: None,
                    resume_run: None,
                    apply: false,
    ...
    #[test]
    fn interrupted_envelope_is_failed_and_structured() {
        let envelope = interrupted_envelope("vos build");

        assert!(!envelope.ok);
        assert_eq!(envelope.command, "vos build");
        assert!(matches!(envelope.status, CommandStatus::Failed));
        assert_eq!(envelope.payload.kind, "interrupted");
        assert_eq!(envelope.payload.message, "terminated by Ctrl+C");
    }
}
