...
`SpecLab Platform` 是围绕 Spec 驱动实验构建的课程平台。它不把实验当成“提交代码等固定评测”的一次性活动，而是把课程规则、学生设计规格、仓库与工具链、验证与评测、Agent 协作与审计组织成一个可追溯、可验证、可复盘的工程化教学闭环。

对 VeriSpecOSLab 而言，平台要让教师能发布递进式 OS 实验，让学生在受控 Agent 辅助下完成个性化系统构建，并让平台根据设计和证据进行评价，而不是只看最终二进制。
