...
- runtime role、course persona、fixed prompt、mode、tool、skill、MCP 与输出 schema 以 [`11-role-parameter-catalog.md`](./11-role-parameter-catalog.md) 为参数目录。
...
`runHeadlessAgentPrompt` 或共享 runner API 必须支持 per-role system prompt，或提供等价的 fixed prompt 注入机制。`PromptEnvelope` 中的用户 task 不得覆盖 `AgentRoleConfig` 的固定角色规则、tool profile、visibility、schema 和禁止项。
...
调用 `vos-agent`，根据 [`11-role-parameter-catalog.md`](./11-role-parameter-catalog.md) 使用 `GatewayAgent` / `SpecAssistant` fixed prompt。
...
调用 `vos-agent`，根据 role catalog 使用 `SpecCompiler` fixed prompt 与 `spec_compiler_output.v1` schema。
...
调用 `vos-agent`，根据 role catalog 使用 `DebugAgent` fixed prompt 与 `debug_output.v1` schema。
...
`vos-cli` 中的 fixed prompt id 集合必须覆盖 role catalog 的 runtime roles：
...
wrapper 还必须把 role catalog 中的 `tool_profile` 与
`.vos/policy.yaml` 的 `allowed_commands` 做交集，得到本次会话实际
advertised tools 与 `Vos` whitelist。
...
- `ContextBundle` ref
- resolved `AgentRoleConfig` ref
- structured output ref
- tool call summary
- advertised tools、MCP server names 与 loaded skills
- risk flags
