...
本 Lab 把文件、pipe、设备和未来 IPC 收敛为可解释的资源模型，并用 shell 串起前六个 Lab。重点是句柄生命周期、共享规则、错误码和退出回收。
...
Agent 可以审查生命周期状态机、生成并发测试和解释泄漏日志。学生必须决定 ABI 和共享语义。提交 ModuleSpec、InterfaceSpec、实现、shell 演示、泄漏/并发证据、错误码测试与必要 SpecPatch。

## 7. 常见错误

- `dup` 后关闭一个 fd 导致另一个失效：把描述符槽和底层对象引用混为一谈。
- pipe 两端关闭后读者永久睡眠：关闭路径没有唤醒等待队列。
- 进程退出后对象泄漏：只释放了 fd 表，没有递减底层对象引用。
