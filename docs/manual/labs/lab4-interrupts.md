...
> **与教材的关系**：设计维度的详细论述见[教材第 4 章](../book/ch04-interrupts.md)。下表只列本 Lab 要决策的问题；两处如有出入，以教材为准。
...
运行 `vos run qemu` 后，日志应包含定时器计数与外部中断统计。示例（字段以你的内核为准）：

```text
[0] trap vector installed: stvec=0x8020_1000
[0] timer tick: 1
[0] timer tick: 2
[0] uart IRQ: 1 (rx)
[0] IRQ stats: timer=3 uart=1 unknown=0
```

对照门禁：

- 定时器计数单调递增（对应"时钟连续触发，计数单调"门禁）；
- 每个核心都有独立计数行（对应"每核收到预期时钟"）；
- 人为触发一次未知 IRQ 时，日志出现可诊断的失败路径（如 `unknown=1` 或显式错误行），而不是静默忽略；
- 压力测试期间没有死锁（UART 持续输入时内核仍能推进）。
