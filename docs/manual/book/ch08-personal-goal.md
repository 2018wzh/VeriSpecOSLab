...
让你的 OS 从一块真实的磁盘镜像上加载 Linux 根文件系统，并启动其中的 Shell。这是对 Linux ABI 兼容性的终极考验——你要跨越的不只是 syscall 兼容性，还有真实磁盘文件系统格式、真实块设备协议以及两者之间的整合。
...
1. **优先级反转的隐蔽形式。** 不只是显式的锁——中断禁用、自旋锁、甚至一些函数的执行时间过长都能导致优先级反转。系统性地测量所有 `push_off` 区间是更可靠的方法。
