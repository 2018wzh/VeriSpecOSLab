...
> **对应实验**：[Lab 10：验证方法论——从测试结果到可追溯证据](../labs/lab10-verification.md)

> **学完本章你能：**
>
> 1. 建立 Spec ID 到测试证据的完整追溯链，识别"执行成功但没有 `verifies`"的假证据；
> 2. 设计不变量检查与故障注入矩阵，证明检查器会失败（而非永远通过）；
> 3. 用确定性验证工作流（固定种子、有界运行、稳定哈希）产出可重放的验证报告；
> 4. 做一次完整的失败分析：现象、证据、根因、修复与回归链；
> 5. 区分自动检查、QEMU、真实硬件与人工复核的证据分层，按规则遮蔽凭据与绝对路径。
...
**Dijkstra（1968）主张程序正确性可以（而且应该）被证明。** Edsger Dijkstra 在 1968 年发表了一篇只有两页的短文，标题既挑衅又精准：**"Go To Statement Considered Harmful"**（`goto` 语句被认为有害）。文章没有停在"`goto` 有害"这个结论上，重点落在更深一层：**"一个程序的正确性无法通过测试来确立，只能通过对其结构的数学推理来论证。"** 而 `goto` 语句的存在，让这种推理几乎无从谈起。

这篇两页的短文开启了**结构化编程**运动，用 `if/else`、`while`、函数调用代替 `goto`。这些结构化的控制流让你可以**分段证明程序的正确性**：先证明"这个循环结束后变量 x 满足什么性质"，再把这个性质当作下一段代码的前置条件。你写 ModuleSpec 操作条目时用的 `precondition → computation → postcondition` 范式，正是从这里来的。
...
**Hoare（1969）为程序正确性打下了数学基础。** 在 Dijkstra 的宣言发表一年后，牛津大学的 Tony Hoare（后来因"空指针是十亿美元的错误"这句话和 Quicksort 算法的发明而更广为人知）发表了一篇论文。他提出了一组逻辑规则，后来被称为 **Hoare 逻辑**，用数学符号精确地描述"如果 P 在执行代码 C 之前成立，那么 Q 在 C 执行之后成立。"这个 `{P} C {Q}` 三元组，就是你每次写 `precondition → operation → postcondition` 时的理论祖先。
...
Dijkstra 在 1968 年说"程序的正确性应该被证明"。但他说的是几十行的算法，quicksort、Dijkstra 最短路径。把一个**完整的、运行的、有 MMU、中断和并发的操作系统内核**全部形式化验证，这在当时被认为是做不到的。

2009 年，NICTA（澳大利亚国家 ICT 研究所）的团队完成了 seL4 微内核的形式化验证，这是人类历史上第一个被完整证明"实现满足规范"的通用操作系统内核。他们在 Isabelle/HOL 证明助手中写了约 20 万行形式化规范，证明了约 9000 行 C 代码在形式化模型下没有任何 bug（包括不会死锁、不会空指针、不会缓冲区溢出等）。从 Dijkstra 1968 年的两页宣言，到 seL4 2009 年的 20 万行证明，这个领域走了 40 年。

seL4 验证的关键洞见：**形式化验证解决不了所有问题，但它会逼你把"正确"的含义定义清楚。** 形式化规范本身就是一份极其精确的设计文档，每个函数的输入约束、输出保证、不变量都在数学上无歧义。开发过程中，大量设计缺陷在写规范阶段就被发现了，不必等到实现之后。

> **原始文献：** G. Klein et al., "seL4: Formal Verification of an OS Kernel," *Proceedings of the 22nd ACM Symposium on Operating Systems Principles (SOSP)*, pp. 207-220, October 2009. 这篇论文获得了 SOSP 最佳论文奖。注意作者列表有 15 人，这是一个团队的成果，也反映出形式化验证 OS 的复杂度和工作量。

但 seL4 也暴露了形式化验证的边界。验证覆盖了内核的 C 实现，但不覆盖硬件（CPU、MMU 可能不符合规范），不覆盖编译器（GCC 可能引入 bug），也不覆盖用户态代码。形式化验证能让你在精确模型内获得极高信心，模型之外的假设仍然存在。

### 对你这门课的意义

你不会做形式化验证，那需要专门的工具和大量时间。但你正在做的事直接承接这条线索：写 ModuleSpec 定义精确的 precondition/postcondition，写不变量检查器在运行时持续验证。你写下的每一个 `assert(freelist has no duplicates)`，都在实践 Dijkstra 在 1968 年提出的命题：**"程序正确性不能靠测试来证明，只能靠精确的结构化推理。"** 你用的工具是 C 的 `assert` 和运行时检查，不是数学定理，精神却一脉相承。
...
验证"模块拼在一起后仍然正确"。你的 DesignSpec 的组合不变量中定义的跨模块不变量属于这一层。

### 层次 4：设计一致性

验证“你的设计文档内部没有矛盾”。`vos spec lint` 负责结构、引用、路径、稳定 ID 和 manifest 映射的确定性校验；`vos agent review` 可以补充建议，更高层的设计判断仍由学生和教师完成。
...
不要直接写代码。先不引用任何实现细节，写下你的分配器应该满足的精确性质：
...
不要在每次分配/释放时都运行所有检查，有些检查是 O(n) 的（比如遍历整个 freelist）。分两层：
...
|------|---------|
| 不变量检查器 | 至少 5 个可运行且通过 |
| 跨组件不变量 | 至少 1 个已在 DesignSpec 的组合不变量中定义且可验证 |
...
**与阶段 8 的关联**：如果你选了 O4（安全加固），fuzzer 就是你的"红队"，专门负责打破你的安全防线。fuzzer 无 crash 是你的 O4 方向的核心 benchmark。

#### 挑战 B：性能回归检测

你的 OS 一直在演化，每次添加新功能，都可能让已有路径的性能悄悄倒退。
...
**教学价值**：性能回归检测能帮你养成"测量→优化→再测量"的科学习惯。你的优化到底有没有效，数据说了算，不靠直觉。
...
4. **SpecPatch 无 before/after**：只记录了"我改了 X"，没记录从什么改成什么，这样的记录不是 SpecPatch，只是 changelog。

## 参考卡：不变量、故障注入与可复现调试

不变量检查器先定义可观察状态和独立 oracle，再选择操作内断言、阶段性扫描或跨模块验收。每个关键检查器至少注入一次应失败的故障，保存原始日志、注入点、预期/实际结果、修复和定向回归；不要用“测试通过”替代 oracle。

用固定输入和明确的 build/test/QEMU/hardware 命令重放；GDB 断点、寄存器、串口和产物身份应能回指失败假设。验证报告还要复盘跨平台契约：哪些性质依赖 HAL，哪些平台假设仍未抽象。

### 调试方法论：从症状收敛到根因

把调试分成五层：先用思维实验写出可被证伪的假设；再用不变量或最小串口标记缩小范围；然后二分启动阶段、调用链或日志区间；必要时用 GDB 停在入口、trap、页错误和锁等待处；最后把失败命令、代码/Spec 身份、原始日志和定向回归整理成证据。每次只改变一层，不能用“换工具链、改链接地址、改 UART”三件事一起掩盖症状。

常见现象先按边界分类：空串口通常先查镜像是否加载、入口/栈/BSS，再查 UART 设备和参数；启用分页后崩溃要同时核对页表根、权限、地址范围和 TLB 刷新；中断不来要区分未使能、控制器未确认、trap 上下文破坏和调度未消费；文件写入不一致要追踪块缓存、DMA 所有权、日志提交和恢复顺序。GDB 的 `target remote`、`break`、`stepi`、`info registers`、`x`、`bt` 只是通用骨架，具体寄存器、地址和异常编码必须查当前 ISA、QEMU 和板卡文档。

调试记录至少保留：复现输入、第一次可观察的失败点、被排除的假设、根因、修复的契约或代码、定向回归和完整回归。若只在 QEMU 成功，要补充说明真实板卡、缓存、时钟、IRQ 或 DMA 的差异；平台值应回到 HAL/平台实现集中管理，而不是在调试补丁中复制。
