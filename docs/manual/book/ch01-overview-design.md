...
**Specs 手册** 在你写规格时随时查阅。它说明 DesignSpec、ModuleSpec、InterfaceSpec、GoalSpec 和 SpecPatch 分别怎么写。
...
除了四本手册，这门课还有一个独特的工具：项目级 AI Agent（基于 `vos-agent`）。它不是通用聊天 AI，受三层约束：
...
**DesignSpec 是逐步生长的，不是一次填满的。** 在 Lab 1 就要求你决定内核架构、参考系统和设计目标有三个问题：没有上下文的选择只能是猜（你连分页机制都没见过就要判断"参考 Sv39 还是 5-level paging"）；早期决策推翻时没有记录机制（代码改了但 seed 没人更新）；一次性填满像填表（学生倾向于把 seed 当成作业模板而不是设计工具）。正确的做法是：Lab 1 只填身份信息（项目名、平台、语言），后续每个 Lab 在真正面对设计问题时才填写对应的 seed 字段，你写下的每个决策都绑着一个你亲手撞过的墙。
...
flag 只是一种确定性验收目标。源码不能硬编码期望值，公开证据也不能泄露学生唯一 flag。完整要求见 [Lab 1](../labs/lab1-seed.md) 与 [CTF 附录](../appendices/ctf-flags.md)。
...
6. **对双环境差异的第一次实践**：Lab 1 的 CTF 热身要求同一份 flag 分别经过 Linux 文件接口和裸机读取路径，帮助你把“操作系统提供了什么”落到实际代码与证据。

进入 Lab 1 之前，你能回答这几个问题吗：你选的 ISA 的 syscall 指令叫什么？你选的语言在 OS 开发中需要禁用哪些特性（如 C++ 的异常和 RTTI、Rust 的 std）？DesignSpec 的哪些字段在 Lab 1 填、哪些在 Lab 5 填？
