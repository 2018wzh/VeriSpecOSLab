...
"你自己的"意味着这个 OS 的设计决策是你做的。它参考了哪些已有系统、拒绝了哪些概念、选择了什么资源模型、追求什么目标。这些都不是教学团队替你做好的选择题。你的 DesignSpec 是你全部后续设计的锚点。
...
**1950s：批处理的诞生。** 把多个程序攒成一批，一个接一个地跑，中间不用人插拔电缆。这叫"批处理监控程序"，是操作系统的雏形。但它解决的是效率问题，还顾不上易用性。程序员提交一叠卡片，几小时后取回结果，中间完全看不到程序在跑。
...
在 Lab 1，你只需要理解"操作系统是什么"并选择语言和 ISA。宏内核还是微内核、fd-based 还是 capability-based、参考什么系统，这些具体的架构决策会在后续 Lab 中逐渐做出，并通过逐 Lab 更新 DesignSpec 来记录设计的演化。你选择宏内核，你站在 Linux 和 xv6 的肩膀上。你选择微内核，你站在 Minix 和 seL4 的肩膀上。**你不知道历史，你的选择就是随机的。你知道历史，你的选择就是有理由的设计判断。**
...
Lab 1–8 使用项目级 AI Agent（基于 `vos-agent`）。它不是通用聊天 AI，受三层约束：
...
这三层约束服务于前八个 Lab 的学习目标：Agent 可以解释不同 ISA 的差异、对比语言的优劣、审查规格格式，但不会替你决定架构，也不会在没有规格的情况下替你写核心代码。Lab 9 起，Codex、Claude Code、Gemini CLI、Copilot 等 Coding Agent 可以直接修改项目；限制从“工具能不能写”转为“学生能不能说明改了什么、为什么这样改、如何验证”。
...
对 RISC-V 不熟的话，花半小时浏览 [RISC-V 参考](http://riscvbook.com/chinese/RISC-V-Reader-Chinese-v1.pdf)，不需要背，混个脸熟就行。
...
**DesignSpec 是逐步生长的，不是一次填满的。** 在 Lab 1 就要求你决定项目身份、参考系统和设计目标的边界：没有上下文的选择只能是猜（你连分页机制都没见过就要判断"参考 Sv39 还是 5-level paging"）；早期决策推翻时没有记录机制（代码改了但 seed 没人更新）；一次性填满像填表（学生倾向于把 seed 当成作业模板而不是设计工具）。正确的做法是：Lab 1 先填写身份、canonical board、已验证的启动/控制台方式和范围边界，后续每个 Lab 在真正面对设计问题时才填写对应的 Spec 字段，你写下的每个决策都绑着一个你亲手撞过的墙。
