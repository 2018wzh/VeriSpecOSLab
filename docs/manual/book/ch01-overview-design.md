...
> 本章 §1.1–§1.8 是课程概述，帮你理解这门课要做什么、OS 是什么。§1.9–§1.15 聚焦 lab1 的核心决策：为什么先设计、你的 OS 回答哪四个问题、选什么语言和平台、怎么写 DesignSpec。内核架构对比、资源模型范式、参考系统剖析这些更深入的设计原理，已分布到后续各章（第 5 章和第 7 章），在你需要做具体决策时出现。

> **对应实验**：[Lab 1：CTF 热身与项目初始化](../labs/lab1-seed.md)

> **学完本章你能：**
>
> 1. 解释操作系统在裸机程序与应用程序之间扮演的角色，说出"从零构建 OS"这条路径与改教学内核的本质区别；
> 2. 为你的 OS 选择语言、ISA 与开发环境，并说清每个选择的主要取舍；
> 3. 写出一份可通过 `vos spec lint` 的 DesignSpec，覆盖目标、non-goals、组合不变量与平台边界；
> 4. 区分 DesignSpec（系统约束）与后续 ModuleSpec（实现契约），知道 QEMU 与真实板卡各自的证据地位；
> 5. 配置 vos CLI、Agent 与知识库，完成 Lab 1 的项目初始化并产出可追溯的 Git 提交。
...
"你自己的"意味着这个 OS 的设计决策是你做的。它参考了哪些已有系统、拒绝了哪些概念、选择了什么资源模型、追求什么目标。你的 DesignSpec 是你全部后续设计的锚点。

最后你会得到一个能在 QEMU 和真实硬件上运行的完整操作系统，以及一套描述它的设计规格、验证证据和演化记录。让这个操作系统真正变成你自己的东西。

## 1.2 操作系统是什么——一个简短的历史视角

理解操作系统最好的方式不是背定义，是看它怎么一步一步变成今天这样的。一切的设计并不是空穴来风，而是在特定的时代背景下前人根据特定的需求做出的决策。

**1940s-1950s：没有操作系统的时代。** 程序员直接操作硬件，插拔电缆、设置开关。一台计算机一次只跑一个程序。程序崩溃？整台机器停摆，程序员自己排查。资源利用率这个词还没出现，机器大部分时间在等人。

**1950s：批处理的诞生。** 把多个程序攒成一批，一个接一个地跑，中间不用人插拔电缆。这叫"批处理监控程序"，是操作系统的雏形。但它解决的是效率问题，还顾不上易用性。程序员提交一叠卡片，几小时后取回结果，中间完全看不到程序的运行过程。
...
**1970s：Unix 的时代。** Ken Thompson 和 Dennis Ritchie 在贝尔实验室写了 Unix，最初只是为了在 PDP-7 上玩一个叫 Space Travel 的游戏。Unix 带来了几个影响深远的设计决定：一切皆文件（用统一的文件描述符操作文件、设备和管道）、层级文件系统、Shell 作为普通用户程序、管道（`|`）作为 IPC 原语。这些决定并非"显然正确"，它们是在无数替代方案中被证明简洁而强大的。Unix 的哲学凝结成一句话：**"Do one thing and do it well."**
...
在 Lab 1，你只需要理解"操作系统是什么"并选择语言和 ISA，然后你可以通过我们的实验来熟悉你选择的语言，架构与开发板的操作。宏内核还是微内核、fd-based 还是 capability-based、参考什么系统，这些具体的架构决策会在后续 Lab 中逐渐做出，并通过逐 Lab 更新 DesignSpec 来记录设计的演化，后续的每一步决策都由你在学习后根据自己的理由做出设计判断。

## 1.3 什么是操作系统——从裸机编程到操作系统的认知跃迁

在讨论内核架构、Unix 哲学和微内核论战之前，有一个更基本的问题需要回答：**操作系统到底是什么？它解决了什么问题？**

如果你有单片机裸机编程的经验（比如 STM32、Arduino），这个问题可能会更加直观。你的程序直接操作硬件，跑得很好，为什么还需要一个叫"操作系统"的东西插在你和硬件之间？
...
- 时钟控制寄存器（RCC）的基地址：`0x40021000`，以及 GPIOA/GPIOC 的时钟使能位，它为硬件设备提供“心跳”，让信息能够编码成电信号发送。
- GPIO 端口的基地址：`0x40010800`（GPIOA）、`0x40010C00`（GPIOC），这个设备是用来点亮LED的。
- UART 的基地址：`0x40013800`（USART1），以及波特率寄存器的计算公式，这个设备是用来与外界传递文本信息的。
...
不教 Linux 内核源码阅读。你的 OS 和 Linux 没什么关系，除了你可能在 DesignSpec 中声明借鉴了它的某些概念。

不教你从零写汇编。你会在启动阶段写几十行汇编入口代码，但不需要先成为汇编专家。

不教你成为 C 语言高手。清晰的、能正确表达设计意图的 C 代码，胜过技巧性的、让人看不懂的 C 代码。

但有一件事你必须愿意做：在想不清楚的时候停下来，画张图，写段 Spec。闷头写代码碰运气，在 OS 开发中不好使。OS 的 bug 往往在你写代码几个小时后才炸，你几乎不可能单靠"让代码跑起来"来定位根因。

## 1.5 课程怎么组织的

十个阶段，大致对应一个 OS 从无到有的自然生长顺序。

阶段 1 定义你要构建什么，并连接第一块真实板卡。阶段 2 让内核启动，输出第一行字。阶段 3 管理内存。阶段 4 响应中断。阶段 5 让用户程序跑起来，这是最长的阶段，涉及 trap、进程、syscall、调度。阶段 6 让数据持久化。阶段 7 决定你的 OS 以什么方式把系统能力暴露给用户程序，这也是你架构中最核心的分叉路口。阶段 8 注入个性，通过方向组合定义你的 OS 的多维剖面。阶段 9 把系统移植到 Lab 1 已确定的真实硬件。阶段 10 建立验证体系：不变量检查器、证据管理、验证密度。Final Lab 综合验收与答辩。
...
**再按规格实现。** Lab 1–8 使用受控的 VOS Agent 辅助规格审查、候选实现和故障诊断。Lab 9 起可以直接使用通用 Coding Agent 修改代码、测试、构建文件和 Spec，课程不再规定工具的角色或调用顺序，但你仍要能解释每个设计选择并验证结果。
...
学生教材只有 Book 和 Labs 两部分，各阶段各有一份；命令、平台和调试要点直接放在对应正文的参考卡中。
...
规格字段、工具命令、QEMU/GDB 调试和 ISA 查阅路径已经放进各章与 Lab 的参考卡。正文只保留完成当前实验所需的概念、检查方法和短示例；完整标准和寄存器定义应回到架构规范、QEMU 文档、板卡手册或工具官方文档。

Lab 1–8 的日常节奏是打开 Book 理解设计空间，用 `vos agent ask` 讨论取舍，打开 Lab 手写 Spec，运行 lint 和只读 review，修改并手动提交后，再由 `vos agent implement <module>` 生成候选实现与测试。Lab 9 起可以把同一过程交给你选择的 Coding Agent，但仍要保留设计理由、测试结果、失败分析和最终 Spec。每个阶段的重点都是审查差异和证据，而不是记住某个工具的固定命令。

### AI Agent 的角色

Lab 1–7 使用项目级 AI Agent（基于 `vos-agent`）。它不是通用聊天 AI，受三层约束：
...
这三层约束服务于前八个 Lab 的学习目标：Agent 可以解释不同 ISA 的差异、对比语言的优劣、审查规格格式，但不会替你决定架构，也不会在没有规格的情况下替你写核心代码。Lab 8 起，Codex、Claude Code、Gemini CLI、Copilot 等 Coding Agent 可以直接修改项目；限制从“工具能不能写”转为“学生能不能说明改了什么、为什么这样改、如何验证”。
...
DesignSpec 不是"交作业"。它是你的设计锚点，从 Lab 1 的身份信息开始，随每个 Lab 逐步长出完整的设计图景。每次你发现自己在两个可行方案之间纠结的时候，回到 DesignSpec 看你的 goals 和 constraints，答案通常就在那里。
...
**资源抽象与复用。** CPU 只有一个（或几个），跑的程序有几十个。你怎么让每个程序都"觉得"自己独占 CPU？基本思路是快速切换：一个程序跑几毫秒，然后切换到下一个。切换太快了，人类感知不到。内存也一样，物理内存是一块连续的 RAM，但每个程序有自己的"虚拟地址空间"，看到的是连续的、私有的内存。
...
你的 DesignSpec 不需要在"资源复用"上标新立异，快速切换这个思路是共享的。你的设计选择主要落在"隔离的强度"和"接口的形态"上。

### 1.10.2 问题二：你的 OS 为什么而存在？

**教学目的。** 你的 OS 存在是为了让你学会系统设计的原理。这意味着"代码清晰"比"跑得快"优先级高。你可能会做一个效率不高的分配器、一个简单的调度器，只要它们的设计理由在你的 设计理由 中说清楚了，这就够了。
...
这些目标可以共存但要排优先级。阶段 5 你发现一个"教学清晰"和"兼容 POSIX"冲突的设计选择，怎么办？如果你在 DesignSpec 中写了"教学目标优先于兼容目标"，答案就已经在那里了。

### 1.10.3 问题三：你的 OS 跑在什么上？

RISC-V 64 + QEMU `virt` 是本课程的技术默认。但这不意味着你被绑死在 RISC-V 上。你的 DesignSpec 中的 `target_platform` 字段可以声明任何 ISA，你只需要理解不同 ISA 之间的关键差异。
...
启用分页后，修改页表必须通知 CPU "你缓存的地址翻译过期了"，也就是刷新 TLB。三种 ISA 的 TLB 维护机制差异是 Lab 3（内存管理）的核心知识点。
...
选择什么语言，在 DesignSpec 的 `constraints` 字段中声明。无论选什么，内核都需要与硬件直接交互，这是所有语言的共同挑战。
...
所有 MMIO 操作、页表操作、上下文切换都是 `unsafe`。但如果你的 `unsafe` 块太大（一个 `unsafe` 包裹了整个 `syscall` 函数），你实际上放弃了 Rust 的安全保证。如果你用 Rust 写内核，你的 DesignSpec 应该声明 `unsafe` 的使用策略，每个 `unsafe` 块应最小化，并在文档注释中解释为什么编译器不能证明这段代码的安全性。
...
进入正式内核主链之前，Lab 1 保留了一次双环境热身：从同一个文件系统镜像读取两个 flag，在 Linux 中通过文件接口完成，在裸机中自己建立块读取、格式解析和 UART 输出。

这不是为了提前实现完整文件系统，也不是把轮询交替输出包装成调度器。它把第 1.3 节的对比变成可运行证据：Linux 程序只需要调用文件接口，裸机程序却要面对镜像布局、设备访问、链接地址和串口寄存器。等你在 Lab 6 再实现可写文件系统时，就能清楚区分“课程提供的只读热身框架”和“内核负责的完整持久化子系统”。

flag 只是一种确定性验收目标。源码不能硬编码期望值，公开证据也不能泄露学生唯一 flag。CTF 源码、镜像、日志和报告都在仓库外完成并手工提交；实板只验证连接、启动和串口挑战标记。完整要求见 [Lab 1](../labs/lab1-seed.md)。

### 1.12.1 平台边界和 HAL

Lab 1 就开始记录平台边界。把板卡、启动介质、串口和设备描述的来源写在 DesignSpec 或实板报告里，后续再决定哪些信息由设备树、ACPI、固件接口或平台配置提供。核心代码不应直接依赖某一块板卡的 MMIO 地址、IRQ 编号或时钟频率；这些值可以在平台适配层出现，但要集中管理并说明适用范围。

这条原则会贯穿后面的实验：

- 启动层封装入口状态、早期控制台和关机；
- 内存层区分 RAM/MMIO 发现、页表格式、TLB 刷新和缓存约束；
- 中断层隔离 IRQ 控制器、定时器、IPI 和架构 trap 上下文；
- 用户空间层隔离上下文切换、特权级转换和 syscall ABI；
- 文件系统和设备层通过块设备、DMA 和缓存一致性边界避免绑定 QEMU；
- 验证时用跨平台契约和来源记录检查仍未抽象的假设。

HAL 不是一套必须照抄的目录结构，也不要求所有值都动态发现。你要回答的是：换一块板卡时，哪些代码应保持不动，哪些平台实现需要替换，当前固定值为什么足够，以及它会把什么迁移成本留到后面。

## 1.13 预览：DesignSpec 会变成什么样

**在 Lab 1，你填写身份信息和硬件起点**：项目名、目标平台、编程语言、canonical board、一句话摘要，以及已经验证过的启动和控制台方式。goals、non_goals、reference_systems 等深层次字段在后续 Lab 中逐步填充，你一定在真正面对那些设计问题的时候才写，而不是在还没理解分页机制的时候"猜"。
...
- DesignSpec 的系统目标和内核组织是否提供了可执行方向？（“教学用宏内核，优先可观察性”好过“一个高效的操作系统”）
- 你选的语言和 ISA 有理由吗？（"因为教程多"是一个好理由；"因为大家都在用"不一定是，你了解过这些人为什么选这个吗？）

## 1.14 常见陷阱

**目标过大。**"我要做一个比 Linux 更好的 OS"，这句话在 DesignSpec 里不能出现。不是因为不对，是因为不可操作。把目标缩小到你能在一学期内验证的程度。
...
4. **对 DesignSpec 的预期**：它不是一次填满的表格，而是随 10 个 Lab 逐步生长的设计锚点（§1.9）。当前严格字段和完整示例见 [Lab 1](../labs/lab1-seed.md)；这里先理解它要承载的设计问题。
5. **对 AI Agent 角色的认识**：Lab 1–8 受三层约束；Lab 9 起允许直接使用 Coding Agent，但设计理由、证据和报告责任仍由学生承担（§1.7）。
6. **对双环境差异的第一次实践**：Lab 1 的 CTF 热身要求同一份 flag 分别经过 Linux 文件接口和裸机读取路径，并增加真实板卡连接和串口挑战标记，帮助你把“操作系统提供了什么”落到实际代码与证据。

进入 Lab 1 之前，你能回答这几个问题吗：你选的 ISA 的 syscall 指令叫什么？你选的语言在 OS 开发中需要禁用哪些特性（如 C++ 的异常和 RTTI、Rust 的 std）？DesignSpec 的哪些字段在 Lab 1 填、哪些在 Lab 5 填？哪些硬件假设已经有来源，哪些仍应由 HAL 或固件信息承接？

能回答，说明本章已经帮你建立了足够的设计视角。打开 [Lab 1 实验卡片](../labs/lab1-seed.md)，开始动手。

## 参考卡：工具链与查阅路径速查

- [Lab 1 实验卡片](../labs/lab1-seed.md)：项目初始化、CTF 热身、Agent 配置与知识库。
- [术语表](../glossary.md)：课程特有术语与 OS 常用术语，标注首次出现位置。
- 工具链安装与验证：Lab 1 参考卡的"内核工具链安装卡"（分平台命令 + 版本下限 + `--version` 验证）；vos CLI 安装见 Lab 1 步骤 1。
- QEMU 交互热键：`Ctrl-a x` 退出、`Ctrl-a c` 进 monitor，详见 Lab 1 参考卡"QEMU 交互热键卡"。
- 查阅路径：ISA 特权级/页表/中断 → 对应架构手册；QEMU 机器参数 → `qemu-system-* --help` 与机器文档；板卡寄存器 → canonical board 手册与设备树；Spec 字段 → Lab 卡片中的字段骨架与 `vos spec lint` 错误信息。
- 参考实现：`examples/xv6-spec` 子模块（参考源码可读，不作为保密边界）。
