...
**Secure Boot 解决什么问题？** 前文所述的启动链有一个隐含假设：固件加载的每一个二进制文件都是可信的。但如果有人物理接触了你的电脑，把硬盘上的内核换成了一个恶意版本呢？或者更隐蔽的——替换了 bootloader，让它加载恶意内核？Secure Boot 的答案是：**在启动链的每一级，验证下一级的数字签名，签名通过才执行。**
...
1. **信任根（Root of Trust）**：平台制造商在出厂时将一组公钥（PK——Platform Key、KEK——Key Exchange Key、db——允许的签名数据库、dbx——禁止的签名数据库）写入固件的非易失性存储（NVRAM）。这些数据在物理上受保护——普通软件无法修改。
