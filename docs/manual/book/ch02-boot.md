...
> **对应实验**：[Lab 2：最小内核启动——用 Spec 描述从硬件到第一条指令](../labs/lab2-boot.md)

> **学完本章你能：**
>
> 1. 讲清从硬件复位到内核第一条指令的整条路径：固件职责、复位向量、ELF 加载与链接脚本；
> 2. 写出最小启动序列的 ModuleSpec：入口状态、栈、BSS 清零、多核启动与输出通道；
> 3. 配置 `vos.yaml` 的 QEMU runner，用 `success_pattern` 与 `failure_pattern` 判定启动成功；
> 4. 用 GDB 断点、寄存器快照和串口标记定位"内核不输出任何东西"的阶段（§2.8a 四阶段排查）；
> 5. 区分固件直启、Bootloader 与 UEFI 三种路径，说清 QEMU 能力与真实板卡启动链的差异。
...
本节将启动链拆解为八个层层递进的主题（含一节选读）。先回答"固件为什么存在"，再讲"从按下电源到 `_start` 到底发生了什么"，然后回答"固件在运行时能为我做什么"，接着通过裸机/BIOS/UEFI 的三方对比说明固件的必要性，用 ELF 格式讲清楚内核镜像的结构，最后落到启动路径选择和 UART 输出，把原理接回实践。
...
这就是"固件"诞生的背景。它出现的目的很朴素：把程序员从拨开关的体力劳动里解放出来。
...
> **原始文献：** Gary A. Kildall, "CP/M: A Family of 8-and 16-Bit Operating Systems," *BYTE Magazine*, vol. 6, no. 6, pp. 216-246, June 1981. Kildall 在这篇文章中详细解释了 CP/M 的分层设计哲学。这篇文章是在 IBM 选择了微软的 MS-DOS 而非 CP/M 之后发表的。这是一个改变了计算机行业格局的商业决策（BIOS 的名字留了下来，但 CP/M 的创始人被排除在了 PC 革命之外）。

**ROM 和 IBM PC BIOS 的出现。** 1981 年，IBM PC 5150 引入了第一套大规模商用的固件方案。IBM 把一个 8 KB 的 ROM 芯片焊在主板上，映射到物理地址 `0xFE000`~`0xFFFFF`。这正是今天 x86 CPU 从 `0xFFFF0` 开始执行的由来：复位向量落在 ROM 的末尾，只有 16 字节的空间放一条 `JMP` 指令跳到真正的 BIOS 代码。IBM 的工程师直接采用了 CP/M 的"BDOS + BIOS"架构思路：MS-DOS 的 `IO.SYS` 就是 BIOS 层，`MSDOS.SYS` 就是 BDOS 层。而固化在 ROM 中的"系统 BIOS"是 IBM 的创新：把硬件初始化的最底层代码烧死在芯片上，让操作系统不必关心主板特定细节。
...
4. 时钟分发到 CPU 各核心、内存控制器、外设总线。直到这一步，"数字逻辑"才开始运转
...
固件的第一条指令通常是一条 `JMP`，因为复位向量处只预留了 16 字节（x86）或类似的小空间，放不下完整的固件入口逻辑。这条 `JMP` 跳到固件的真正入口。
...
> **为什么固件需要几百 KB 甚至几 MB？** 不是因为"启动逻辑"本身复杂。启动逻辑可能只有几十 KB。体积膨胀来自硬件驱动：每个主板可能使用不同厂商的内存控制器、不同的 Super I/O 芯片、不同的 PCIe 根端口配置。UEFI 固件（如 TianoCore/EDK II）本质上是一个为启动优化的迷你操作系统：它有完整的驱动程序模型、文件系统支持（FAT32），甚至网络栈（PXE 启动）。
...
OpenSBI 实现运行在 M-mode，你的内核运行在 S-mode。内核通过 `ecall` 指令请求 M-mode 服务。这跟"用户态通过系统调用请求内核服务"是同一种机制，只是角色换成了"内核请求固件"。
...
**关键约束**：`ExitBootServices()` 是一把剪刀。调用之后，Boot Services 全部失效。但 Runtime Services 仍然可用。你的内核必须在调用 `ExitBootServices()` 之前，把物理内存映射、帧缓冲地址等关键信息保存到自己的数据结构中。这条边界是 UEFI 启动阶段最重要的设计约束。
...
> **对零基础自学者的启示**：如果你觉得 OS 启动流程"步骤太多记不住"，不要硬记。回到上面那张三方差异表，每一步都是为了解决"裸机程序不需要面对、但 OS 必须面对"的问题。BSS 清零？因为裸机的数据段在 Flash 中已知初值，而 OS 内核在 RAM 中运行，RAM 上电后内容是随机的。设置栈？裸机也设，但启动文件替你做了，OS 得自己来，因为固件不知道你的内核打算把栈放在哪。请求固件服务？裸机不需要，整块芯片都是你的；OS 需要，因为固件已经占用了最高特权级，你要通过规定的接口请它代劳。
...
ELF（Executable and Linkable Format）被设计成同时服务于两个截然不同的目的：链接和执行。它用两套平行的"目录"来实现这一点：
...
这些符号**不是变量**。它们没有存储空间，只是地址标签。在 C 中引用它们时，这是一个非常容易出错的点：
...
2. **在 freestanding 环境中没有 CRT**。在宿主环境（Linux/Windows 用户程序）中，C 运行时（`crt0.o`）会在 `main()` 之前调用 `memset(__bss_start, 0, __bss_size)`。在你的内核里没有 CRT，这一步得自己做。
...
无论你用 C 还是 Rust，核心道理是一样的：**BSS 清零 + `memset`/`memcpy`（C）或 `panic_handler`（Rust）构成你的最小语言运行时。** 除此之外的一切，包括 `printf`、`malloc`、文件操作，都可以等到后面阶段再实现。但这几个函数必须在阶段 2 就位，否则你的内核要么链接不过，要么行为随机。
...
**x86-64 学生建议**：绝对不要手动写 16 位实模式到 64 位 long mode 的切换代码。选择 GRUB（Multiboot2 协议）或 Limine。GRUB 工具包里的 `grub-mkrescue` 可以把内核镜像和 GRUB 一起打包成可启动的 ISO 文件（需要先安装 GRUB 本体与构建工具）；Limine 的 `limine` 工具同样简洁。你花在阅读 Multiboot2 规范上的三十分钟，省掉的是在 GDT 表项出错时对着三重故障抓狂的三个下午。
...
这个阶段的成果谈不上复杂，它验证了你的工具链、链接布局和硬件理解是否正确，搭起一个**可复现、可验证的基础执行环境**，后续所有阶段都建立在这个基础上。
...
2. **MMIO 访问不能缓存。** CPU 访问 DRAM 时，数据可能在 L1/L2 cache 中就被截获了。但设备寄存器的值会"自己变化"。LSR 的 THRE 位随时可能从 0 变成 1。如果把设备寄存器的值缓存在 CPU cache 里，你读到的永远是过时数据。这就是为什么设备地址范围必须被标记为不可缓存（在 RISC-V 上通过 PMA，即 Physical Memory Attributes 配置），以及为什么 C 代码中必须用 `volatile` 指针。
...
**为什么这条路径在启动阶段特别重要。** 在整个 OS 开发过程中，UART 输出是你唯一的调试手段。没有屏幕、没有日志文件、没有调试器 GUI。你看到的第一行 `Hello, OS!`，是"电源键按下"到"UART 数据寄存器被写入"这条漫长路径的终点。理解这条路径的每一个环节，从供电、时钟、复位向量、固件初始化、ELF 加载、BSS 清零、栈设置到 UART 寄存器写入，是排查"内核不输出任何东西"问题的前提。出了问题，你不会对着空白的终端发呆；你会从 `_start` 开始，顺着这条路径，逐个环节验证。

### 2.2.8 ⚡ 选读：Secure Boot——启动链的信任根

> 本节为选读内容，不影响主线实验的完成。但它解释了为什么现代计算机比二十年前更难被"邪恶 maid"攻击，以及你的内核在真实硬件上启动时面临的信任问题。

**Secure Boot 解决什么问题？** 前文所述的启动链有一个隐含假设：固件加载的每一个二进制文件都是可信的。但如果有人物理接触了你的电脑，把硬盘上的内核换成了一个恶意版本呢？或者更隐蔽地替换了 bootloader，让它加载恶意内核？Secure Boot 的答案是：**在启动链的每一级，验证下一级的数字签名，签名通过才执行。**
...
│                                                      │
│  ┌──────────┐     ┌──────────┐     ┌──────────┐     │
│  │ 固件 ROM  │────→│ Bootloader│────→│  内核    │     │
│  │ (信任根)  │验证 │ (签名验证) │验证 │ (签名验证) │     │
│  └──────────┘     └──────────┘     └──────────┘     │
│       ↑                  ↑                  ↑        │
│   硬件固化          公钥嵌入          公钥嵌入       │
│   不可修改          固件中            bootloader中   │
└─────────────────────────────────────────────────────┘
```

**UEFI Secure Boot（x86-64/ARM64 的事实标准）。** UEFI 规范定义了 Secure Boot 的具体实现：

1. **信任根（Root of Trust）**：平台制造商在出厂时将一组公钥（PK，即 Platform Key；KEK，即 Key Exchange Key；db，允许的签名数据库；dbx，禁止的签名数据库）写入固件的非易失性存储（NVRAM）。这些数据在物理上受保护，普通软件无法修改。
2. **Bootloader 验证**：UEFI 固件在加载 `\EFI\BOOT\BOOTX64.EFI` 之前，先计算其哈希，在 `db` 中查找匹配的签名。如果找不到，拒绝启动。
3. **内核验证**：Bootloader（如 GRUB 的 `shim` + `grubx64.efi`）在加载内核之前，同样验证内核镜像的签名。如果签名无效或已被吊销（在 `dbx` 中），拒绝加载。
4. **吊销机制**：当发现某个签名证书已被破解（如 2022 年的 GRUB 缓冲区溢出漏洞 `CVE-2020-14372`），平台制造商发布 `dbx` 更新，将该证书加入黑名单。下次启动时，任何由该证书签名的二进制文件都会被拒绝。

**对教学 OS 的影响。** 在 QEMU 中，Secure Boot 默认关闭，你的内核不需要任何签名就能启动。但如果你想把 OS 部署到真实硬件上：

- **x86-64 UEFI 机器**：大多数主板允许在固件设置中关闭 Secure Boot（"Other OS"模式）。教学 OS 可以在此模式下运行。如果要支持 Secure Boot，需要为内核签名，这通常需要向 Microsoft 提交内核二进制供其签名（因为 Microsoft 的 KEK 预装在大多数 PC 的固件中），或者引导用户进入固件设置手动注册你的公钥。
- **ARM64 设备（如 Raspberry Pi、Surface Pro X）**：Secure Boot 通常强制开启且不可关闭。教学 OS 在这些平台上启动的门槛远高于 x86-64。

**RISC-V 世界的现状。** RISC-V 目前没有统一的 Secure Boot 标准。OpenSBI 在 M-mode 运行，理论上可以在跳转到 S-mode 内核之前验证其签名，但规范尚未标准化。实践中，RISC-V 的 Secure Boot 方案因芯片厂商而异（如 SiFive 的 FU540/C910 有各自的 OTP 密钥存储方案）。对于教学 OS，RISC-V + QEMU 路径完全不需要关心 Secure Boot。

> **一句话总结**：Secure Boot 是启动链的"安检门"，每一级都检查下一级的签名。你的教学 OS 在 QEMU 中不需要它，但理解它有助于解释为什么真实硬件的启动比 QEMU 复杂，以及为什么 2020 年后的 x86-64 机器默认拒绝运行未签名的内核。
...
- 如果你的 DesignSpec 选了 **RISC-V + QEMU virt**：固件直启是最简路径，OpenSBI 已把 CPU 留在 S-mode
...
- **构建系统**：Make、CMake 还是 xtask？它们由工具链 ModuleSpec 管理；`vos build` 只执行 `vos.yaml` 中已经声明的结构化构建目标。
...
`spec/modules/toolchain.yaml`：描述构建、链接、运行的基本参数。
...
- **依赖阶段 1**：你的 DesignSpec 决定了目标平台、ISA 和约束
- **为阶段 3 提供**：你的启动阶段建立了物理内存的初始视图（至少知道哪里是可用 RAM），这将直接成为阶段 3 的输入。如果启动阶段没有正确识别内存布局，页分配器将无法工作。

## 参考卡：链接、启动与早期调试

查阅链接脚本时先确认入口符号、装载地址、段边界、栈和对齐约束，再用 ELF/反汇编工具核对实际产物；不要只凭 QEMU 能启动就认定布局正确。QEMU 的机器型号、内存和串口参数必须与实验记录一致，ISA 启动约定则以对应架构手册和板卡固件文档为准。

把早期串口、关机、固件传入的入口状态和异常停机封装在平台实现中，核心启动流程只依赖稳定接口。GDB 断点、寄存器快照和串口标记应记录为可复现证据；固定地址若确有必要，写明来源、适用平台和集中管理位置。
