...
本节将启动链拆解为七个层层递进的主题。先回答"固件为什么存在"，再讲"从按下电源到 `_start` 到底发生了什么"，然后回答"固件在运行时能为我做什么"，接着通过裸机/BIOS/UEFI 的三方对比说明固件的必要性，用 ELF 格式讲清楚内核镜像的结构，最后落到启动路径选择和 UART 输出，把原理接回实践。
