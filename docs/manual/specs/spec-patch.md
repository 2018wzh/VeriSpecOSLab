...
`changes` 引用稳定 Spec ID。VOS 会根据补丁和当前模块 owns 推导影响模块与回归范围；不要求另写一个验证矩阵，也不接受旧的架构阶段文件或隐式兼容入口。跨模块的 `vos agent implement` 必须看到已提交且尚未应用的 SpecPatch，单模块实现无需额外补丁。任一受影响模块的 Agent 实现提交后，这次跨模块授权即告完成；后续再次改变跨模块语义时，应手写并提交新的 SpecPatch，旧补丁不会永久扩大 `owns`。
