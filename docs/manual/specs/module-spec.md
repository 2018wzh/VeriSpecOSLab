...
`owns` 是 Agent 的硬边界：必须覆盖模块实现和模块测试，只能写仓库相对路径，不能使用绝对路径、`..` 或把 Spec、`.git`、`.vos` 运行目录纳入实现所有权。跨模块实现必须把已提交且尚未应用的 SpecPatch 影响模块的 owns 合并后再检查；任一受影响模块的 Agent 实现落地后，这次授权即被消费，不能在后续运行中重复使用。`vos spec lint` 只做确定性校验；`vos agent review` 可以提出等级和契约建议，但不会替学生修改 Spec 或做架构决策。
