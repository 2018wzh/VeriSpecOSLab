...
`owns` 是 Agent 的硬边界：只能写仓库相对路径，不能使用绝对路径、`..` 或把 Spec、`.git`、`.vos` 运行目录纳入实现所有权。跨模块实现必须把已提交 SpecPatch 影响模块的 owns 合并后再检查。`vos spec check` 只做确定性校验；Agent 可以提示等级不足，但不会替学生做架构决策。
