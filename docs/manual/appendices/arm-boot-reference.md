...
本文档提供 ARM 64-bit（AArch64）平台的内核启动约定，供 Lab 2 填写启动 ModuleSpec、工具链 ModuleSpec 和 `vos.yaml` 时参考。
...
工具链 ModuleSpec 应约束入口符号 `_start`、AArch64 LP64 ABI、静态重定位，以及 QEMU `virt` 常用的 `0x40000000` RAM 基址；若使用 U-Boot `booti`，还要声明 Linux kernel image header 约束。`vos.yaml` 只保存可执行投影：
