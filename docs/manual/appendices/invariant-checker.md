...
```yaml
invariants:
  - id: freelist-no-duplicate
    text: freelist 中每个物理页只出现一次
    check: 每次 kalloc 或 kfree 后运行检查器
  - id: allocated-not-in-freelist
    text: 已分配页不存在于 freelist 中
  - id: reserved-never-allocated
    text: 保留区域的物理页从未被分配器返回
```
