...
本文档提供 x86-64 平台的内核启动约定，供 Lab 2 填写启动 ModuleSpec、工具链 ModuleSpec 和 `vos.yaml` 时参考。
...
工具链 ModuleSpec 应约束 `_start`、x86-64 System V ABI、静态重定位、Multiboot2 header 位于镜像前 8 KiB，以及内核加载地址不低于 1 MiB。若使用 UEFI，入口和镜像格式改为 `efi_main` 与 PE/COFF。`vos.yaml` 只保存可执行投影：
