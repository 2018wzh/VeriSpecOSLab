...
在 VeriSpecOSLab 中，链接脚本由工具链 ModuleSpec 管理。`spec/modules/toolchain.yaml` 记录职责、约束和所有权，`vos.yaml` 只记录执行构建与验证的结构化命令。需要新建或修改链接脚本时，先更新工具链 ModuleSpec，再让 `vos agent implement toolchain` 生成候选实现；`vos build` 只负责执行已经声明的构建目标。
