...
### 入口状态（ModuleSpec 顶层 `rely` 与 `entry.pre`）
...
### 栈设置（ModuleSpec 顶层 `guarantee` 与 `entry.post`）
...
### BSS 清零（`bss_zero.pre` 与 `bss_zero.post`）
...
### 非启动核心处理（ModuleSpec 顶层 `concurrency`）
...
### 输出通道（顶层 `rely` 与 `console_output.pre`）
...
### 关机（顶层 `dependencies`、`rely` 与 `shutdown.pre`）
...
### 工具链 ModuleSpec 和 `vos.yaml` 对应值

工具链 ModuleSpec 应约束 `_start`、RISC-V LP64 ABI、静态重定位和 `0x80000000` 的典型内核加载地址。`vos.yaml` 只保存可执行投影，成功条件由 public check 表达：
