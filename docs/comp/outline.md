...
```text
空目录 → init → DesignSpec → ModuleSpec/InterfaceSpec
        → Agent implement → build → public/contract verify
...
Agent Runtime 按 `design/spec/implement/debug/verify/kb/review` 路由。设计和规格先展示 diff 并等待确认；实现只在 detached linked worktree 中运行，检查 clean HEAD、owns、SpecPatch、build 和 public/contract evidence 后才原子应用并提交。worktree 是 Git 回滚边界，不是宿主安全沙箱。
