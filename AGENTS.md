...
The public student surface is intentionally small: `init`, `doctor`, deterministic `spec lint [<Spec ID|path|design|all>]`, command-managed knowledge through `kb add/list/search/remove/clear/export-manifest/import-manifest`, `agent config`, the `agent` roles `implement`, `debug`, `verify`, `ask`, and `review`, and `agent qemu preflight|execute`, plus `build`, `run qemu`, `run hardware`, `verify [--hidden]`, `report`, and `submit`. During Labs 1–8, students discuss choices with `agent ask`, handwrite Specs, run lint and read-only `agent review`, revise them, and commit with ordinary Git commands. From Lab 9 onward, the course also permits external Coding Agents to edit implementation, tests, build files, and Specs directly; the final Spec/evidence/report requirements remain course requirements, but the VOS Agent workflow is no longer a process gate. Keep clean-tree and current-`HEAD` ledger gates only where the active Lab explicitly requires them, including `verify`, `agent implement`, authoritative hardware evidence, and `submit`. Dirty `build` and development QEMU/hardware runs are allowed but their evidence is marked non-submittable. `debug`, `verify`, `review`, and `ask` must not modify project files. Doctor invokes the Debug Agent to derive and probe project tools but treats provider unavailability as a warning.

QEMU port preflight inventories only student-supplied hardware material below `references/qemu/<request-id>/`; missing target-relevant evidence must fail without creating a candidate. Hardware facts must not be filled from the network. Execution may pin official software dependencies, runs in a detached worktree, trusts only the declared structured result for Agent-owned shell acceptance, never updates `vos.yaml`, and never pushes.
