...
The public student surface is intentionally small: `init`, `doctor`, `spec check`, the `agent config` setup command, the seven `agent` roles (`design`, `spec`, `implement`, `debug`, `verify`, `ask`, `review`), `build`, `run qemu`, `run hardware`, `verify`, `report`, and `submit`. Keep clean tree and current `HEAD` ledger gates for `verify`, `agent implement`, authoritative hardware evidence, and `submit`. Dirty `build` and development QEMU/hardware runs are allowed but their evidence is marked non-submittable. `design` and `spec` require confirmation before an atomic commit. `debug`, `verify`, `review`, and `ask` must not modify project files.
